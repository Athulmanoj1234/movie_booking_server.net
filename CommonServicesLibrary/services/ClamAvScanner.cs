...
namespace CommonServicesLibrary.services
{
    public class ClamAvScanner
    {
        ...
        {
            this._clamAvServer = "localhost";
            this._clamAvPort = int.Parse("3310");
        }

        // returns bool value of whether malicious thing is included or not
        public async Task<ClamAvScannedResponse> ScanFileAsync(Stream R2ObjectStream)
        {
            ...
            try
            {
                ...
                using (var client = new TcpClient(this._clamAvServer, this._clamAvPort))
                {
                    using (var netWorkStream = client.GetStream())
                    {
                        ...
                        // chunksize of our actual file bytes 
                        ...
                        {
                            Console.WriteLine($"Sending {readedBytes} bytes to ClamAV");

                            // we need to get the bytes representaton of the bytes readed from the file streaming
                            // for passing the actual bytes of the file first we need to send its size
                            byte[] chunkSize = BitConverter.GetBytes(IPAddress.HostToNetworkOrder(readedBytes));
                            await netWorkStream.WriteAsync(chunkSize, 0, chunkSize.Length);
                        ...
                        while ((bytesRead = await netWorkStream.ReadAsync(responseBuffer)) > 0)
                        {
                            ...
                            {
                                return new ClamAvScannedResponse
                                {
                                    IsScanningCompleted = true,
                                    IsDefaultFound = true,
                                    ScannedMessage = "Scanning completed and malicious resource found"
                                };
                            }
                        }
                        return new ClamAvScannedResponse
                        {
                            ...
                            ScannedMessage = "Scanning completed successfully and no defaults found"
                        };
                    }
                }
            }
            catch (Exception ex)
            {
                return new ClamAvScannedResponse
                {
                    ...
                    IsDefaultFound = false,
                    ScannedMessage = ex.Message
                };
            }
        }
    }

    public class ClamAvScannedResponse
    {
        public bool IsScanningCompleted { get; set; }
        public bool IsDefaultFound { get; set; }
